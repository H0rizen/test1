...
function langdate($format, $stamp, $servertime = false, $custom = false ) {
	global $langdate, $lang, $member_id, $customlangdate;
...
function show_attach($story, $id, $static = false) {
	...
	if( !file_exists( $tpl->dir . "/attachment.tpl" ) ) {
		...
		$tpl->template = <<<HTML
[allow-download]<span class="attachment"><a href="{link}" >{name}</a> [count] [حجم: {size}] ({$lang['att_dcount']} {count})[/count]</span>[/allow-download]
...
function GzipOut($debug=0){
    ...
    if ($ENCODING){
        ...
        if ($debug){
            $s .= "<!-- Debug content: ".strlen($Contents)." Char "; 
            $s .= "Debug: ".strlen(gzencode($Contents, 1, FORCE_GZIP))." Char -->"; 
            $Contents .= $s; 
        }

...
<div>{$lang['upload_t_size']}&nbsp;<input class="edit bk" type="text" style="direction: ltr;" name="t_size" id="t_size" size="9" value="{$config['max_image']}">&nbsp;پیکسل&nbsp;<select name="t_seite" id="t_seite"><option value="0" {$t_seite_selected[0]}>{$lang['upload_t_seite_1']}</option><option value="1" {$t_seite_selected[1]}>{$lang['upload_t_seite_2']}</option><option value="2" {$t_seite_selected[2]}>{$lang['upload_t_seite_3']}</option></select></div>
HTML;
...
<div>{$lang['upload_m_size']}&nbsp;<input class="edit bk" type="text" style="direction: ltr;" name="m_size" id="m_size" size="9" value="{$config['medium_image']}">&nbsp;پیکسل&nbsp;<select name="m_seite" id="m_seite"><option value="0" {$t_seite_selected[0]}>{$lang['upload_t_seite_1']}</option><option value="1" {$t_seite_selected[1]}>{$lang['upload_t_seite_2']}</option><option value="2" {$t_seite_selected[2]}>{$lang['upload_t_seite_3']}</option></select></div>
HTML;
...
if( $member_id['user_group'] == 1 ) {
	...
	$ftp_input = "<div><hr />در این حالت می توانید فایلی که از قبل در پوشه /uploads/files/ آپلود کرده اید را مستقیماً در این خبر آپلود کنید. فقط کافیست نام آن را وارد کنید:<br/><input class=\"edit bk\" type=\"text\" id=\"ftpurl\" name=\"ftpurl\" style=\"width:350px; direction: ltr;\">&nbsp;<b>/uploads/files/</b>&nbsp;<button class=\"edit\" onclick=\"upload_from_url('ftp'); return false;\" style=\"width:115px;\">{$lang['db_load_a']}</button><div id=\"upload-viaftp-status\"></div></div>";
...
<input type="hidden" name="user_hash" value="{$dle_login_hash}" />
	<div id="stmode">
		...
		<div><hr />{$lang['images_upurl']}&nbsp;<input class="edit bk" type="text" id="copyurl" name="copyurl" style="width:300px; direction: ltr;">&nbsp;<button class="edit" onclick="upload_from_url('url'); return false;" style="width:115px;">{$lang['db_load_a']}</button><div id="upload-viaurl-status"></div></div>
		{$ftp_input}
...
<div style="clear: both;"></div>
<div>
<div class="properties">{$lang['images_align']}&nbsp;<select id="imagealign" name="imagealign" style="width: 110px;">
...
<table>
	<tr>
		...
		<td><input id="imageurl" name="imageurl" value="" style="width:460px;" class="edit bk ltr" /></td>
	</tr>
	<tr>
...
jQuery(document).ready(function ($) {
	...
	var uploader = new qq.FileUploader({
		...
		onProgress: function(id, fileName, loaded, total){
					$('#uploadfile-'+id+' .qq-upload-size').text( 'میزان آپلود شده: ' + uploader._formatSize(loaded)+' {$lang['media_upload_st8']} '+uploader._formatSize(total));

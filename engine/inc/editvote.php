...
if( $_GET['action'] == "views" AND $_GET['id']) {
	...
	for($i = 0; $i < sizeof( $body ); $i ++) {
		...
		$entry .= "<div align=\"right\">$body[$i] - تعدا آرا: $num ($proc%)</div><div class=\"voteprogress\" align=\"right\"><span class=\"vote{$pn}\" style=\"width:".intval($proc)."%;\">{$proc}%</span></div>\n";
  ...
  <div class="box-content">
	...
	<div class="row box-section">
		...
		<div class="form-group">
		  ...
		  <div class="col-lg-10">
			<input id="PersianDate_A" type="text" name="start_date" size="20" value="{$start_date}" class="ltr" />&nbsp;<span class="help-button" data-rel="popover" data-trigger="hover" data-placement="right" data-content="{$lang['hint_vstart']}" >?</span>
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-lg-2">{$lang['vote_enddate']}</label>
		  <div class="col-lg-10">
			<input id="PersianDate_B" type="text" name="end_date" size="20" value="{$end_date}" class="ltr" />&nbsp;<span class="help-button" data-rel="popover" data-trigger="hover" data-placement="right" data-content="{$lang['hint_vend']}" >?</span>
		  </div>
		 </div>
...
while ( $row = $db->get_row() ) {
	...
	$item_id = $row['id'];
	$item_date = jdate( "Y/m/d", strtotime( $row['date'] ) );

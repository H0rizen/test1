...
HTML;
	...
	while ( $row = $db->get_array() ) {
		...
		$row['date'] = jdate( "d M Y ساعت H:i:s", $row['date'] );
		$status = $lang["admin_logs_action_".$row['action']];

		$entries .= "
        <tr>
        <td width=\"220\">{$row['date']}</td>
        <td align=\"center\"><a class=\"status-info\" onclick=\"javascript:popupedit('".urlencode($row[name])."'); return false;\" href=\"#\">{$row['name']}</a></td>

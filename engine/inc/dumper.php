...
class dumper {
	...
	function backup() {
		...
		if(!defined('AUTOMODE'))
		{
		  ...
		  $name = $db . '_' . jdate("Y-m-d_H-i"). '_' . substr( md5(jdate("Y-m-d_H-i").DBHOST . DBNAME), 0, 5);

		} else {

		   $name = jdate("Y-m-d_H-i") . '_' . $db . '_' . md5($rand);
        ...
        foreach ($tables AS $table){
			...
			echo tpl_l("{$lang['dumper_11']} `{$table}` [با " . fn_int($tabinfo[$table]) . " داده].");
        	// Создание таблицы

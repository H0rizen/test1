...
while ( $row = $db->get_array() ) {
	...
	$row['date'] = strtotime( $row['date'] );
	$date = jdate( "d M Y ساعت H:i", $row['date'] );
 ...
 <div class="avatar"><input name="selected_comments[]" value="{$row['id']}" type="checkbox"></div>
    <div class="info">
      <span class="name">
        <span class="label label-green">{$lang['edit_autor']}</span> <strong class="indent">{$row['autor']}</strong> آی پی: {$row['ip']} {$lang['cmod_n_title']} در خبر <strong>{$news_title}</strong>
      </span>

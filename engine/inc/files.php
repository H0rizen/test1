...
foreach ( $images_in_dir as $file ) {
	...
	if( in_array( $img_type, $allowed_extensions ) AND is_file( $config_path_image_upload . $file ) ) {
	  ...
	  <td align='center'>$img_info[0]x$img_info[1]</td>
	  <td align='center'>" . formatsize( $this_size ) . "</td>
...
if( !$total_size ) {
	...
	echo "</tbody></table><div class=\"box-footer padded\">
		<div id=\"file-uploader\" style=\"width:210px;float:right;\"></div>{$lang['images_listdir']} {$folder_list}
		<div style=\"float:left;\">حجم فایل‌ها در این پوشه: " . formatsize( $total_size ) . " <input class=\"btn btn-red\" type=\"submit\" value=\" {$lang['images_del']} \"><input type=\"hidden\" name=\"action\" value=\"doimagedelete\"><input type=\"hidden\" name=\"user_hash\" value=\"$dle_login_hash\" /></div>
	</div>";
